import os
import argparse
import cv2
import mediapipe as mp

#function to detect faces
def detect_faces(img, face_detection):

    H, W, _ = img.shape

    rgb_image = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    out = face_detection.process(rgb_image)

    if out.detections is not None:
        for detection in out.detections:
            location_data = detection.location_data
            bbox = location_data.relative_bounding_box

            x1, y1, w, h = bbox.xmin, bbox.ymin, bbox.width, bbox.height

            x1 = int(x1 * W)
            y1 = int(y1 * H)
            w = int(w * W)
            h = int(h * H)

            # draw rectangle around faces
            img = cv2.rectangle(img, (x1, y1), (x1 + w, y1 + h), (0, 255, 0), 3)

    return img


#to define where to detect faces from
args = argparse.ArgumentParser()

args.add_argument("--mode", default='video')
args.add_argument("--filePath", default='mkbhd.mp4')

args = args.parse_args()

# create a directory named "output" if it doesn't exist
output_dir = './output'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# detect faces
mp_face_detection = mp.solutions.face_detection

with mp_face_detection.FaceDetection(model_selection=0, min_detection_confidence=0.5) as face_detection:

    #to detect faces from image
    if args.mode in ["image"]:
        # read image
        img = cv2.imread(args.filePath)

        img = detect_faces(img, face_detection)

        # save image
        cv2.imwrite(os.path.join(output_dir, 'output.png'), img)

    #to detect faces from video
    elif args.mode in ['video']:

        cap = cv2.VideoCapture(args.filePath)
        ret, frame = cap.read()

        #create a video writer object to create the new face detected video file
        output_video = cv2.VideoWriter(os.path.join(output_dir, 'detected_output.mp4'),
                                       cv2.VideoWriter_fourcc(*'MP4V'),
                                       25,
                                       (frame.shape[1], frame.shape[0]))

        while ret:

            frame = detect_faces(frame, face_detection)

            output_video.write(frame)

            ret, frame = cap.read()

        cap.release()
        output_video.release()

    #to detect faces from webcam
    elif args.mode in ['webcam']:
        cap = cv2.VideoCapture(0)

        ret, frame = cap.read()
        while ret:
            frame = detect_faces(frame, face_detection)

            cv2.imshow('frame', frame)
            cv2.waitKey(25)

            ret, frame = cap.read()

        cap.release()